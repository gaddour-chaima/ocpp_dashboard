      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4 mt-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{t.settings.colorModeDesc}</p>
              </div>
              <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
                    theme === 'light'
                      ? 'bg-white dark:bg-slate-200 text-slate-800 shadow-sm'
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{t.settings.languageDesc}</p>
              </div>
              <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                <button
                  onClick={() => setLang('en')}
        ...
        <div className="card p-5">
          ...
          <div className="space-y-4 mt-4">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 italic">{t.settings.notifNote}</p>
          </div>
        </div>
      </div>
    </div>
...
function SectionTitle({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="flex items-center gap-2">
      <div className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center">{icon}</div>
...
function ToggleSwitch({ enabled }: { enabled: boolean }) {
  return (
    <div
      className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors flex-shrink-0 ${enabled ? 'bg-blue-500' : 'bg-slate-200 dark:bg-slate-700'}`}
    >
      <div
